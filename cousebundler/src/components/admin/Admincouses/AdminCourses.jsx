...
function AdminCourses() {
  ...
  const CourscedetailsHandler = (userid) => {
    onOpen();
    console.log(userid);
  }
  ...
  const addLectureHandler = (e, id, title, discription, video) => {
    e.preventDefault();
  };
        ...
        <TableContainer w={['100vw', 'full']}>
          ...
          <Table variant={'simple'} size="lg">
            ...
            <Tbody>
              {
                courses.map(item => (
                  <Row CourscedetailsHandler={CourscedetailsHandler} delethandler={delethandler} key={item.id} item={item} />
                ))
              }
...
function Row({ item, CourscedetailsHandler, delethandler }) {
  return (
    <Tr>
      <Td>{item.id}</Td>
      <Td>{item.Title}</Td>
      ...
      <Td>{item.Catatagory}</Td>
      <Td>{item.Creator}</Td>
      <Td isNumeric>{item.views}</Td>
      <Td isNumeric>{item.noOfVideos}</Td>
      <Td isNumeric>
        <HStack justifyContent={'flex-end'}>
          <Button onClick={() => CourscedetailsHandler(item.id)} color={'purple.500'} variant={'outline'}>View lectures</Button>
